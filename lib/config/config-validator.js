...
const { InvalidConfigError } = require('../common/errors')
  ...
  errors
    .map((error) => {
      if (error.keyword === 'additionalProperties') {
        ...
        if (['globals', 'env', 'parserOptions'].includes(formattedPropertyPath)) {
          return `Unexpected top-level property "${formattedPropertyPath}" in solhint config is no longer supported since version 4.0.0`
        }
...
const validate = (config) => {
  ...
  if (!validateSchema(config)) {
    throw new InvalidConfigError(formatErrors(validateSchema.errors))
  }
