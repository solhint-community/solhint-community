...
class NoUnusedVarsChecker extends BaseChecker {
  ...
  'FunctionDefinition:exit'(node) {
    // we skip adding a var usage scope for function _declarations_ and
    // functions with an empty / without a code block
...
class VarUsageScope {
  static of(node) {
    ...
    if (node.type === 'FunctionDefinition') {
      ...
      functionNode = findFirstParent(node, (it) => !!it.funcScope)
    }
